...
cmd:option('-save',"log_exp_6CVPR_Cifar10_NoW" ,'subdirectory to save logs')
...
cmd:option('-nIter',  3,'for WDBN_PI_Sym methods')
cmd:option('-momentum_u',0.1,'for FSN methods, when using RA, the momentum')
...
baseString=opt.model..'_depth'..opt.depth
         ...
         ..'_wD'..opt.weightDecay
         ..'_nIter'..opt.nIter..'_lD'..opt.learningRateDecay..'_dr'..opt.dropout
         ..'_seed'..opt.seed
...
--results.Norm_Output=Norm_Output
   torch.save('result_6CVPR_Cifar10_Now_'..baseString..'.dat',results)
end
