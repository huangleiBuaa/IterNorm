...
function M.parse(arg)
   ...
   cmd:option('-data',       '/data/lei/imageNet/input_torch/',         'Path to dataset')
   ...
   ---------- Model options ----------------------------------
   cmd:option('-model',      'imagenet/resnet_BN', 'Options: resnet | preresnet')
   ...
   cmd:option('-eps',   1e-7  ,'')
   cmd:option('-T',   1000  ,'')
   cmd:option('-maxUpdate',   20  ,'')
   cmd:option('-N_SVD_updateEpoch',   30  ,'')
   cmd:option('-LrFactor',   100  ,'')
   cmd:option('-nIter',   5  ,'the number of powerIter')
...
---------------------------for resnext--------------
   ...
   cmd:option('-momentum_BN',        0.1,   'momentum for BN running average')
   ...
   opt.rundir = cmd:string('console/exp_2_imagenet/Info', opt, {dir=true})
